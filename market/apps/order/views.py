...
from car.helper  import get_car_key, json_msg
...
class ConfirmOrder(VerifyLoginView):
    ...
    def get(self, request):
        ...
        # >>>> 1. 收货地址处理
        address = UserAddress.objects.filter(user_id=user_id).order_by("-isDefault")
        # address = None
        ...
        for sku_id in sku_ids:
            ...
            try:
                ...
                # 商品不存在就回到购物车
                return redirect("car:购物车展示")

            # 获取对应商品的数量
            try:
                count = r.hget(car_key, sku_id)
                count = int(count)
            except:
                # 商品不存在就回到购物车
                return redirect("car:购物车展示")
